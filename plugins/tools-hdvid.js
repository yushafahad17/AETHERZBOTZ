...
await conn.sendMessage(m.chat, {
    ...
    ffmpeg.ffprobe(outputPath, async (err, metadata) => {
      ...
      ffmpeg(outputPath)
        ...
        .outputOptions('-crf', '1') // Atur nilai crf sesuai kebutuhan (semakin rendah, semakin tinggi kualitas)
        .save(outputPath.replace('.mp4', '_hd.mp4'))
        .on('end', () => {
          conn.sendFile(m.chat, outputPath.replace('.mp4', '_hd.mp4'), '', `📹 Nih Njeng Udah Gua Bikin HD`, m)
        })
        .on('error', (err) => {
          console.error(err)
