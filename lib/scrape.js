...
return await videyUp(await(m.quoted.download()));
...
function facebook(url) {
    return new Promise(async (resolve, reject) => {
        ...
        axios.post(host + '/system/action.php', qs.stringify(form.data), { headers: is.headers })
            .then(({ data }) => {
                if (data.links.lenght == 0) return resolve({ creator: '@aether – AethersCode', status: false })
                resolve({ creator: '@aether – AethersCode', status: true, data: data.links })
            })
    })
}

module.exports = { facebook, quotes, igdl, igstory, tiktok, twitter, joox, pin, videy }
